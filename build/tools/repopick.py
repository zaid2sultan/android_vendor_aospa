...
if __name__ == '__main__':
    # Default to AtomXCLO Gerrit
    default_gerrit = 'https://review.atomx-devs.org'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from AtomXCLO's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit atomx', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch atomx', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
